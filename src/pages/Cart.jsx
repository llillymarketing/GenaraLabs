      ...
      <div className="max-w-4xl mx-auto px-6">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-4">
            {items.map((item, i) => (
              <motion.div key={item.product_id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="bg-white border border-[#e2e8f5] rounded-2xl p-4 flex gap-4 shadow-sm">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center justify-between mt-3">
                    <div className="flex items-center bg-[#f8faff] border border-[#e2e8f5] rounded-lg">
                      <button onClick={() => updateQuantity(item.product_id, item.quantity - 1)} className="p-1.5 text-[#8a9bbf] hover:text-[#1a2d5a]"><Minus className="w-3 h-3" /></button>
                      <span className="w-8 text-center text-xs font-medium text-[#1a2d5a]">{item.quantity}</span>
                      <button onClick={() => updateQuantity(item.product_id, item.quantity + 1)} className="p-1.5 text-[#8a9bbf] hover:text-[#1a2d5a]"><Plus className="w-3 h-3" /></button>
                    </div>
                    <div className="flex items-center gap-4">
                      <span className="text-sm font-semibold text-[#1a2d5a]">${(item.price * item.quantity).toFixed(2)}</span>
                      <button onClick={() => removeItem(item.product_id)} className="text-[#8a9bbf] hover:text-red-500 transition-colors"><Trash2 className="w-4 h-4" /></button>
                    </div>
                  </div>
                </div>
              </motion.div>
          ...
          <div>
            <div className="bg-white border border-[#e2e8f5] rounded-2xl p-6 sticky top-24 shadow-sm">
              ...
              <div className="space-y-3 text-sm">
                ...
                {shipping > 0 && <p className="text-xs text-[#8a9bbf]">Free shipping on orders over $200</p>}
              ...
              <Link to={createPageUrl('Checkout')} className="mt-6 w-full flex items-center justify-center gap-2 bg-[#2b7de9] hover:bg-[#3b8ef0] text-white font-semibold py-3 rounded-xl transition-all text-sm shadow-md shadow-[#2b7de9]/20">
