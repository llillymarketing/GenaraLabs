...
export default function Checkout() {
  ...
  const [form, setForm] = useState({
    email: '', name: '', address: '', city: '', state: '', zip: '', country: 'US',
  });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!researchConfirm) {
      alert('You must confirm that this purchase is for research purposes only.');
      return;
    }
    setSubmitting(true);
    const orderNumber = 'GL-' + Date.now().toString(36).toUpperCase().slice(-6);
    // Order creation + email logic handled via Base44 SDK in production
    clearCart();
    setSubmitting(false);
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-2xl p-6">
                ...
                <label className="flex items-start gap-3 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={researchConfirm}
                    onChange={(e) => setResearchConfirm(e.target.checked)}
                    className="mt-1 w-4 h-4 rounded border-amber-300"
                  />
                  <span className="text-sm text-amber-800 leading-relaxed">
                    I confirm that I am purchasing these products <strong className="text-amber-900">for research purposes only</strong>.
                    I understand that these products are <strong className="text-amber-900">not intended for human consumption</strong>,
                    are not FDA approved, and are not dietary supplements.
                  </span>
                </label>
              </div>
            ...
            <div>
              <div className="bg-white border border-[#e2e8f5] rounded-2xl p-6 sticky top-24 shadow-sm">
                ...
                <div className="space-y-3 text-sm border-t border-[#e2e8f5] pt-4">
                  ...
                  <div className="flex justify-between text-[#1a2d5a] font-semibold pt-3 border-t border-[#e2e8f5]">
                ...
                <button
                  type="submit"
                  disabled={submitting || !researchConfirm}
                  className="mt-6 w-full flex items-center justify-center gap-2 bg-[#2b7de9] hover:bg-[#3b8ef0] disabled:opacity-50 disabled:cursor-not-allowed text-white font-semibold py-3.5 rounded-xl transition-all text-sm"
                >
                  {submitting ? (
